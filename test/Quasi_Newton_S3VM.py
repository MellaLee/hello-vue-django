...
def startRun():
	...
	x_train_valid, x_test, y_train_valid, y_test = cross_validation.train_test_split(x, y, test_size=.2,random_state=0)
	x_train, x_valid, y_train, y_valid = cross_validation.train_test_split(x_train_valid, y_train_valid, test_size=.9,random_state=0)
	s3vm = Quasi_Newton_S3VM(x_train, y_train, x_valid)
	print(len(x_train), len(x_valid), len(x_test), len(x))
	s3vm.fit()
	y_score = s3vm.get_predictions(x_test, True)
	...
	plt.plot(fpr, tpr, color='darkorange',
			lw=lw, label='S3VM(area = %0.2f)' % roc_auc) ###假正率为横坐标，真正率为纵坐标做曲线
